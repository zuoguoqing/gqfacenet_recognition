    ...
    for result in results:
        ...
        if result.get("emotion") is not None and len(result["emotion"]) > 0:
            draw.text((int(result["bbox"][0]), int(result["bbox"][1] + 20)), str(result["emotion"]),
                      fill=(255, 255, 255), font=face_recognition.font)
        if result.get("age") is not None and int(result["age"]) > 0:
            draw.text((int(result["bbox"][0]), int(result["bbox"][1] + 40)), str(result["age"]), fill=(255, 255, 255),
                      font=face_recognition.font)
        if result.get("gender") is not None and len(result["gender"]) > 0:
            draw.text((int(result["bbox"][0]), int(result["bbox"][1] + 60)), str(result["gender"]),
                      fill=(255, 255, 255), font=face_recognition.font)
